 ...
 */
static SANE_Status
gl847_bulk_read_data (Genesys_Device * dev, uint8_t addr, uint8_t * data,
		      size_t len)
{
  SANE_Status status;
  size_t target, read;
  ...
  while (target)
    {
      size_t done, size = target > 0xeff0 ? 0xeff0 : target;
...
{
  return (val & REG41_MOTORENB) ? SANE_TRUE : SANE_FALSE;
}
...
get_sensor_profile (int sensor_type, int dpi)
{
  unsigned int i;
  int idx;
 ...
 */
static int
gl847_compute_exposure (Genesys_Device * dev, int xres)
{
		    ...
		    int dpi)
{
  ...
  sanei_genesys_set_reg_from_set (regs, REG_EXPDMY,
				  (uint8_t) ((sensor->expdummy) & 0xff));
 ...
 */
static SANE_Bool
compute_half_ccd (Genesys_Model * model, int ores, int xres)
{
  /* we have 2 domains for ccd: xres below or above half ccd max dpi */
  return ores < 2 * xres && model->flags & GENESYS_FLAG_HALF_CCD_MODE
 ...
 */
static SANE_Status
gl847_init_motor_regs_scan (Genesys_Device * dev, Genesys_Register_Set * reg,
			    ...
			    int scan_power_mode, unsigned int flags)
{
  ...
  DBG (DBG_proc,
       ...
       scan_dummy, feed_steps, scan_power_mode, flags);
  ...
  sanei_genesys_slope_table (scan_table, &scan_steps, scan_yres,
			     scan_exposure_time, dev->motor.base_ydpi,
			     scan_step_type, factor, dev->model->motor_type,
			     gl847_motors);
  RIE (gl847_send_slope_table (dev, SCAN_TABLE, scan_table,
			       scan_steps * factor));
  RIE (gl847_send_slope_table (dev, BACKTRACK_TABLE, scan_table,
			       scan_steps * factor));
  ...
  sanei_genesys_slope_table (fast_table, &fast_steps, fast_dpi,
			     scan_exposure_time, dev->motor.base_ydpi,
			     fast_step_type, factor, dev->model->motor_type,
			     gl847_motors);

  /* manual override of high start value */
  fast_table[0] = fast_table[1];

  RIE (gl847_send_slope_table (dev, STOP_TABLE, fast_table,
			       fast_steps * factor));
  RIE (gl847_send_slope_table (dev, FAST_TABLE, fast_table,
			       fast_steps * factor));
  ...
  sanei_genesys_calculate_zmode2 (use_fast_fed,
				  ...
				  scan_table, scan_steps * factor, feedl,
				  min_restep * factor, &z1, &z2);

  DBG (DBG_info, "%s: z1 = %d\n", __func__, z1);
  sanei_genesys_set_triple (reg, REG60,
			    z1 | (scan_step_type << (16 + REG60S_STEPSEL)));

  DBG (DBG_info, "%s: z2 = %d\n", __func__, z2);
  sanei_genesys_set_triple (reg, REG63,
			    z2 | (scan_step_type << (16 + REG63S_FSTPSEL)));
			      ...
			      int color_filter, int flags)
{
  ...
  Sensor_Profile *sensor;
  unsigned int bytes;
  unsigned int dpiset, dpihw, cksel, factor;
  ...
  segnb = (dev->model->flags & GENESYS_FLAG_SIS_SENSOR) ? dpihw / 600 : 1;
  ...
  if (depth == 1)
    {
      words_per_line = (words_per_line + 7) >> 3;
      dev->len = (dev->len >> 3) + ! !(dev->len & 7);
      dev->dist = (dev->dist >> 3) + ! !(dev->dist & 7);
    }
  else
    {
...
gl847_init_scan_regs (Genesys_Device * dev, Genesys_Register_Set * reg, float xres,	/*dpi */
		      ...
		      unsigned int channels, int color_filter,
		      unsigned int flags)
{
  int bytes_per_line;
  int exposure_time;
  ...
  half_ccd = compute_half_ccd (dev->model, dev->sensor.optical_res, xres);
  ...
  if (flags & SCAN_FLAG_USE_OPTICAL_RES)
    ...
    /* resolution is chosen from a list */
    used_res = xres;
  ...
  scan_step_type =
    sanei_genesys_compute_step_type (gl847_motors, dev->model->motor_type,
				     exposure_time);
  ...
  status = gl847_init_optical_regs_scan (dev, reg, exposure_time, used_res,
  ...
  status =
    gl847_init_motor_regs_scan (dev, reg, exposure_time, slope_dpi,
				scan_step_type,
  ...
  if (depth == 1)
    dev->total_bytes_to_read =
      ((dev->settings.pixels * dev->settings.lines) / 8 +
       ((dev->settings.pixels * dev->settings.lines) % 8 != 0)) * channels;
  else
    dev->total_bytes_to_read =
      dev->settings.pixels * dev->settings.lines * channels * (depth / 8);
...
gl847_calculate_current_setup (Genesys_Device * dev)
{
  ...
  DBG (DBG_info,
       ...
       "Startpos  : %.3f/%.3f\n"
       "Scan mode : %d\n\n",
       __func__, dev->settings.yres, dev->settings.lines,
...
gl847_stop_action (Genesys_Device * dev)
{
  ...
  status = sanei_genesys_read_register (dev, REG40, &home);
  if (status != SANE_STATUS_GOOD)
    {
  ...
  if (!(home & REG40_DATAENB || home & REG40_MOTMFLG))
    {
 ...
 */
GENESYS_STATIC SANE_Status
gl847_rewind (Genesys_Device * dev)
{
  SANE_Status status;
  uint8_t byte;
...
gl847_slow_back_home (Genesys_Device * dev, SANE_Bool wait_until_home)
{
  ...
  dev->settings.scan_mode = SCAN_MODE_LINEART;
  status =
    gl847_init_scan_regs (dev, local_reg, resolution, resolution, 100, 30000,
			  100, 100, 8, 1, 0,
...
gl847_init_regs_for_coarse_calibration (Genesys_Device * dev)
{
  ...
  status =
    gl847_init_scan_regs (dev, dev->calib_reg, dev->settings.xres,
...
gl847_feed (Genesys_Device * dev, unsigned int steps)
{
  ...
  resolution = sanei_genesys_get_lowest_ydpi (dev);
  status =
    gl847_init_scan_regs (dev, local_reg, resolution, resolution, 0, steps,
			  100, 3, 8, 3, dev->settings.color_filter,
			  SCAN_FLAG_DISABLE_SHADING | SCAN_FLAG_DISABLE_GAMMA
			  | SCAN_FLAG_FEEDING |
			  SCAN_FLAG_IGNORE_LINE_DISTANCE);
  if (status != SANE_STATUS_GOOD)
...
gl847_init_regs_for_shading (Genesys_Device * dev)
{
  ...
  dev->calib_resolution =
    sanei_genesys_compute_dpihw (dev, dev->settings.xres);
  dev->calib_lines = dev->model->shading_lines;
  if (dev->calib_resolution == 4800)
    dev->calib_lines *= 2;
  dev->calib_pixels = (dev->sensor.sensor_pixels * dev->calib_resolution) /
  ...
  status =
    gl847_init_scan_regs (dev, dev->calib_reg, dev->calib_resolution,
			  dev->calib_resolution, 0, move, dev->calib_pixels,
			  dev->calib_lines, 16, dev->calib_channels,
			  dev->settings.color_filter,
			  SCAN_FLAG_DISABLE_SHADING | SCAN_FLAG_DISABLE_GAMMA
...
gl847_init_regs_for_scan (Genesys_Device * dev)
{
  ...
  status =
    gl847_init_scan_regs (dev, dev->reg, dev->settings.xres,
...
gl847_send_shading_data (Genesys_Device * dev, uint8_t * data, int size)
{
  ...
  if (DBG_LEVEL >= DBG_data)
	...
	fprintf (dev->binary, "P5\n%d %d\n%d\n",
		 (endpixel - strpixel) / factor * channels, lines / channels,
		 255);
    }
  ...
  for (i = 0; i < 3; i++)
      ...
      for (x = 0; x < pixels; x += 4 * factor)
	{
      ...
      status =
	sanei_genesys_write_ahb (dev->dn, dev->usb_mode, addr, pixels,
				 buffer);
      if (status != SANE_STATUS_GOOD)
	{
...
gl847_led_calibration (Genesys_Device * dev)
{
  ...
  status =
    gl847_init_scan_regs (dev, dev->calib_reg, used_res, used_res, 0, 0,
...
gl847_init_gpio (Genesys_Device * dev)
{
  ...
  /* search GPIO profile */
  while (gpios[idx].sensor_id != 0 &&
	 dev->model->gpo_type != gpios[idx].sensor_id)
    idx++;
...
gl847_init_memory_layout (Genesys_Device * dev)
{
  ...
  idx = 0;			/* default to canon-lide-100 */
 ...
 * @param forward SANE_TRUE if searching forward, SANE_FALSE if searching
 * backward
 * @param black SANE_TRUE if searching for a black strip, SANE_FALSE for a white
 * strip
 * @return SANE_STATUS_GOOD if a matching strip is found,
 * SANE_STATUS_UNSUPPORTED if not
 */
static SANE_Status
gl847_search_strip (Genesys_Device * dev, SANE_Bool forward, SANE_Bool black)
{
  ...
  status =
    gl847_init_scan_regs (dev, local_reg, dpi, dpi, 0, 0, pixels, lines,
			  depth, channels, 0,
			  SCAN_FLAG_DISABLE_SHADING |
			  SCAN_FLAG_DISABLE_GAMMA);
  if (status != SANE_STATUS_GOOD)
  ...
  while (pass < 20 && !found)
      ...
      if (DBG_LEVEL >= DBG_data)
	{
	  sprintf (title, "search_strip_%s_%s%02d.pnm",
		   black ? "black" : "white", forward ? "fwd" : "bwd",
		   (int) pass);
	  sanei_genesys_write_pnm_file (title, data, depth, channels, pixels,
					lines);
	}
      ...
      /* when searching forward, we only need one line of the searched color since
       * we will scan forward. But when doing backward search, we need all the
	...
	for (y = 0; y < lines && !found; y++)
	  {
	    ...
	    for (x = 0; x < pixels; x++)
		  ...
		  (!black && data[y * pixels + x] < 60))
		count++;

	    /* at end of line, if count >= 3%, line is not fully of the desired
	     * color so we must go to next line of the buffer */
	    /* count*100/pixels < 3 */
	    if ((count * 100) / pixels < 3)
	      {
		found = 1;
		DBG (DBG_data,
		     "%s: strip found forward during pass "
		     "%d at line %d\n", __func__, pass, y);
	      }
	    else
...
gl847_offset_calibration (Genesys_Device * dev)
{
  ...
  status =
    gl847_init_scan_regs (dev, dev->calib_reg, resolution, resolution, 0, 0,
  ...
  RIEF2 (gl847_begin_scan (dev, dev->calib_reg, SANE_TRUE), first_line,
	 second_line);
...
gl847_coarse_gain_calibration (Genesys_Device * dev, int dpi)
{
  ...
  /* no gain nor offset for AKM Analog Front End */
  RIE (sanei_genesys_read_register (dev, REG04, &reg04));
  if ((reg04 & REG04_FESET) == 0x02)
    {
      DBGCOMPLETED;
